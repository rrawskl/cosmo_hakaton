...
- Протонные измерения покрывают только наблюдённое время. Для будущего окна
  нет выдуманного прогноза: фактор остаётся insufficient_data. Текущая обстановка
  показана отдельно. Архив GOES за май–июнь 2024 не подключён; replay честно
  сообщает DATA_UNAVAILABLE для протонов, сохраняя NOAA прогноз и орбиту.
- Общий результат — объяснимое сравнение факторов без суммы рисков 0–100.
  Неполнота исключает уверенного победителя; при равенстве получается equal.
  Тень — условие работ, не автоматический запрет EVA.
