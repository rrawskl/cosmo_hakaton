...
Панель GOES в текущем режиме имеет кнопки **6 часов / 1 день / 3 дня / 7 дней**;
по умолчанию 6 часов. Это прошедшие наблюдения rolling operational API,
а не длительность EVA и не прогноз. Переключение обновляет ряды, максимумы,
время максимумов и источники. Панель работает и при открытом сохранённом
расчёте; его исходные измерения остаются неизменными в JSON/PDF.

В историческом режиме проверяется весь выбранный интервал. Если он целиком
попадает в последние 7 дней и уже завершился, панель отбирает только точки
этого интервала. Иначе вместо графика показывается DATA_UNAVAILABLE / UNKNOWN.
Произвольный исторический архив GOES не подключён; отсутствие измерений
не означает 0 pfu или NORMAL. Полный исторический расчёт backend по-прежнему
ограничен маем–июнем 2024: для него используется доступный прогноз S1 NCEI,
а современные измерения GOES не подставляются.

Активные правила не формируют категорию adverse. Поэтому карточки сравнения
скрывают неинформативные «Неблагоприятно: 0 мин», сохраняя минуты внимания.
Это ограничение модели, а не доказательство безопасности; старые результаты
с фактическим adverse продолжают показывать эту категорию.
